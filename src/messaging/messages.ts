...
export const captureSubmitSchema = z.object({
  type:z.literal('leetx/capture-submit'), captureId:z.string().min(8), platform:platformSchema, problemKey:z.string().min(1), title:z.string(), canonicalUrl:z.string().url(), accountKey:z.string().min(1), language:z.string(), code:z.string().min(1).max(MAX_CODE_LENGTH), codeHash:z.string().min(8), captureMethod:z.enum(['editor-model','textarea','rendered-code','manual']), captureConfidence:z.enum(['high','medium','low']), submittedAt:z.number().int().positive(), sourceUrl:z.string().url(), issues:z.array(z.string()), verdict:verdictSchema.exclude(['pending']).optional(), rawVerdict:z.string().min(1).optional(), runtimeText:z.string().optional(), memoryText:z.string().optional(), verdictAt:z.number().int().positive().optional()
});
