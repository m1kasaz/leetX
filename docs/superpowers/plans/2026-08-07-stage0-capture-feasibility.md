...
正式界面以 `entrypoints/app/` 中已落地的工作台为唯一视觉基线,打开后直接进入产品工作台,不增加营销介绍、功能引导或演示封面。整体采用深色极客风格,背景色 `#080b13`,面板色 `#0e1420`,主强调色 `#b9f45c`,错误色 `#ff6c7a`,LeetCode 辅助色 `#ffb65f`,洛谷辅助色 `#61d9dd`;正文使用 Manrope,代码与元数据使用 DM Mono,面板圆角 10–12px,边框使用低透明度白色。
