...
3. 打开扩展工作台页面，查看历史记录、提交时间线与相邻提交的代码差异（支持 Python、C/C++、Java、JavaScript/TypeScript、Go、Rust、C#、Kotlin、Swift 语法高亮）。
4. 在工作台中选中一次提交，点击「分析」手动触发 AI 分析：结果以**最小改动 diff** 呈现——红绿行级对比标出 AI 建议的改动，悬浮改动行会在分析栏左侧浮出该行的解释（可通过「行内解释」开关关闭）。AI 分析需要你在扩展设置中配置自己的 API Key，仅在你明确点击时请求，费用可控。
